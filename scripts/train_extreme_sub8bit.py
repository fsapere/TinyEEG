...
# Define custom 4-bit weight quantizer by subclassing Int8WeightPerChannelFloat
...
# ==========================================
# 1. QUANTIZED EEGNET ARCHITECTURE (INT4)
# ==========================================
class Sub8EEGNet(nn.Module):
    def __init__(self, n_chans, n_outputs, n_times, F1=8, D=2, F2=16):
        super(Sub8EEGNet, self).__init__()
...
# ==========================================
# 2. QUANTIZED EEG CONFORMER ARCHITECTURE (INT4)
# ==========================================
class Sub8EEGConformer(nn.Module):
    def __init__(self, n_chans, n_outputs, n_times):
        super(Sub8EEGConformer, self).__init__()
...
def calculate_model_footprint(model):
    ...
    Weights of quantized layers: 4-bit signed integer (flat, 0.5 bytes)
    ...
    for name, param in model.named_parameters():
        ...
        if is_quant_weight:
            total_bytes += numel * 0.5  # 4-bit standard weight = 0.5 bytes
        elif is_quant_bias:
            total_bytes += numel * 4    # 32-bit standard bias = 4 bytes
        else:
...
def main():
    ...
    # ----------------------------
    # A. Train INT4 Standard EEGNet
    # ----------------------------
    print("\n" + "="*50)
    print("A. TRAINING INT4_STANDARD_PERCHANNEL EEGNET FROM SCRATCH")
    print("="*50)
    eegnet_model = Sub8EEGNet(n_chans=n_chans, n_outputs=n_outputs, n_times=n_times).to(device)
    eegnet_save_path = '/scratch/fsapere/TinyEEG/models/eegnet_sub8_best.pth'
    ...
    log_to_csv("EEGNet_INT4_Standard_PerChannel", eegnet_best_acc, eegnet_params, eegnet_footprint)
    
    # ----------------------------
    # B. Train INT4 Standard EEGConformer
    # ----------------------------
    print("\n" + "="*50)
    print("B. TRAINING INT4_STANDARD_PERCHANNEL EEG CONFORMER FROM SCRATCH")
    print("="*50)
    conformer_model = Sub8EEGConformer(n_chans=n_chans, n_outputs=n_outputs, n_times=n_times).to(device)
    conformer_save_path = '/scratch/fsapere/TinyEEG/models/conformer_sub8_best.pth'
    ...
    log_to_csv("EEGConformer_INT4_Standard_PerChannel", conformer_best_acc, conformer_params, conformer_footprint)
    
    print("\nTraining completed successfully! (INT4 Standard Sub-8-bit simulations saved)")
